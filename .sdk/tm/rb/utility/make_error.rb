...
module MullvadVpnUtilities
  MakeError = ->(ctx, err) {
    if ctx.nil?
      ...
      ctx = MullvadVpnContext.new({}, nil)
    end
    op = ctx.op || MullvadVpnOperation.new({})
    opname = op.name
    opname = "unknown operation" if opname.empty? || opname == "_"

    result = ctx.result || MullvadVpnResult.new({})
    result.ok = false

    err = result.err if err.nil?
    err = ctx.make_error("unknown", "unknown error") if err.nil?

    errmsg = err.is_a?(MullvadVpnError) ? err.msg : err.to_s
    ...
    sdk_err = MullvadVpnError.new("", msg, ctx)
    ...
    sdk_err.code = err.code if err.is_a?(MullvadVpnError)

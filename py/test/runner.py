...
class MullvadVpnTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            MullvadVpnTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = MullvadVpnTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = MullvadVpnTestRunner.getenv("MULLVAD_VPN_TEST_LIVE")
        override = MullvadVpnTestRunner.getenv("MULLVAD_VPN_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = MullvadVpnTestRunner.getenv(key)
        ...
        explain = MullvadVpnTestRunner.getenv("MULLVAD_VPN_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["MULLVAD_VPN_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if MullvadVpnTestRunner._test_control is not None:
            return MullvadVpnTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                MullvadVpnTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            MullvadVpnTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = MullvadVpnTestRunner.load_test_control()
...
def load_env_local():
    MullvadVpnTestRunner.load_env_local()


def env_override(m):
    return MullvadVpnTestRunner.env_override(m)


def entity_data(v):
    return MullvadVpnTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return MullvadVpnTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return MullvadVpnTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return MullvadVpnTestRunner.load_test_control()


def live_delay_ms():
    return MullvadVpnTestRunner.live_delay_ms()

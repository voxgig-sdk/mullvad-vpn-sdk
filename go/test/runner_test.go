...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("MULLVAD_VPN_TEST_LIVE") == "TRUE" ||
		os.Getenv("MULLVAD_VPN_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("MULLVAD_VPN_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.MullvadVpnSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.MullvadVpnError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.MullvadVpnSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.MullvadVpnError{Msg: msg, Code: code}
}

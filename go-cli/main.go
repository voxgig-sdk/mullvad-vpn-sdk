// Command MullvadVpn-cli is a CLI + REPL for the MullvadVpn SDK,
...
const prompt = "mullvad-vpn"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "ip_information"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      MULLVAD_VPN_APIKEY (api key), MULLVAD_VPN_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: MULLVAD_VPN_APIKEY carries the API key and
	// MULLVAD_VPN_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("MULLVAD_VPN_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("MULLVAD_VPN_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewMullvadVpnSDK(opts)

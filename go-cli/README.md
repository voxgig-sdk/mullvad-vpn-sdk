...
boru-driven command-line client **and** interactive REPL for the MullvadVpn
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/mullvad-vpn-cli)
make build

# 2. See usage (words, entities, env vars)
./mullvad-vpn-cli --help

# 3. Provide credentials once, via the environment
export MULLVAD_VPN_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./mullvad-vpn-cli load 1 ip_information            # {id:1} shorthand
./mullvad-vpn-cli load '{id:1}' ip_information       # explicit match map

# 5. Override the API base URL for a single call
MULLVAD_VPN_BASE=https://api.example.com ./mullvad-vpn-cli load 1 ip_information

# 6. No arguments -> interactive REPL
./mullvad-vpn-cli
mullvad-vpn> load 1 ip_information
mullvad-vpn> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/mullvad-vpn-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/mullvad-vpn-cli load 1 ip_information
   ```
...
```sh
./mullvad-vpn-cli load 1 ip_information          # scalar shorthand for {id:1}
./mullvad-vpn-cli load '{id:1}' ip_information     # explicit match map
```
...
./mullvad-vpn-cli load 1 ip_information
```
...
Run with no arguments to open a REPL (prompt `mullvad-vpn>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `ip_information`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/mullvad-vpn-cli`. |
...
(`./mullvad-vpn-cli <expr>`) and interactively (the REPL), and expressions compose the

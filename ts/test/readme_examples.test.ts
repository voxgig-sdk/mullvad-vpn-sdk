...
const TEST_SEED = {"entity":{"ip_information":{"test01":{"id":"test01"}}}}
...
function isRunnable(code: string): boolean {
  return (
    /new\s+MullvadVpnSDK\b/.test(code) ||
    /\bMullvadVpnSDK\.test\b/.test(code) ||
    /\bclient\s*\./.test(code)
  )
}
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+MullvadVpnSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/MullvadVpnSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)

...
module MullvadVpnUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = MullvadVpnResponse.new({})
    ...
    if fetch_err
      ...
      response = MullvadVpnResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = MullvadVpnResponse.new(fetched)
    else

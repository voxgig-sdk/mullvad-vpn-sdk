...
require_relative 'MullvadVpn_types'
...
class MullvadVpnSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = MullvadVpnHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = MullvadVpnHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, MullvadVpnFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    MullvadVpnUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = MullvadVpnHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = MullvadVpnHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = MullvadVpnHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => MullvadVpnError.new(
        "#{op}_allow",
        "MullvadVpnSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = MullvadVpnHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = MullvadVpnError.new(
        "graphql_error", "MullvadVpnSDK: graphql: #{msg}")
      res["graphql"] = errors
    end

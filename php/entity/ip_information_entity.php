...
class IpInformationEntity
{
    ...
    {
        if ($args) {
            $this->_data = MullvadVpnHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = MullvadVpnHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *   SDK boundary; throws MullvadVpnError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = MullvadVpnHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }

...
class FtCtrl extends MullvadVpnControl
{
...
class FtClient
{
    ...
    private MullvadVpnUtility $utility;

    public function __construct(MullvadVpnUtility $utility, array $options)
    {
    ...
    public function get_utility(): MullvadVpnUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (MullvadVpnContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public MullvadVpnContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (MullvadVpnContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = MullvadVpnConfig::make_feature($name);
    ...
    private static function buildUrl(MullvadVpnSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof MullvadVpnError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new MullvadVpnResponse($fetched);
        }
    ...
    private function fail(MullvadVpnContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (MullvadVpnContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;

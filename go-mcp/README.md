...
[MCP](https://modelcontextprotocol.io) server exposing the MullvadVpn SDK as
two agent tools — `mullvad-vpn_list` and `mullvad-vpn_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/mullvad-vpn-mcp)
make build

# 2. Provide credentials via the environment
export MULLVAD_VPN_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user mullvad-vpn \
  -- /absolute/path/to/mullvad-vpn-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./mullvad-vpn-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// mullvad-vpn_list: first page of records
{ "entity": "ip_information" }
{ "entity": "ip_information", "query": { } }

// mullvad-vpn_load: one record by id
{ "entity": "ip_information", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/mullvad-vpn-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user mullvad-vpn \
     -- "$PWD"/dist/*/mullvad-vpn-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `mullvad-vpn_list` and `mullvad-vpn_load` tools now appear
   in new sessions. Ask the agent to *"list ip_information using mullvad-vpn"*
   and it calls `mullvad-vpn_list` with `{"entity":"ip_information"}`.
...
```sh
export MULLVAD_VPN_APIKEY=sk_live_xxx            # API key
export MULLVAD_VPN_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `mullvad-vpn_list` tool
...
### Call the `mullvad-vpn_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `mullvad-vpn_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `mullvad-vpn_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `MULLVAD_VPN_APIKEY` | API key sent with every request. |
| `MULLVAD_VPN_BASE` | Optional override of the API base URL. |
...
```sh
./mullvad-vpn-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"mullvad-vpn_load","arguments":{"entity":"ip_information","query":{"id":1}}}}'
```
